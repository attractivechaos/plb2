...
fn iit_overlap(a []Interval, max_level i8, st SType, en SType, mut b []Interval) {
	n := a.len;
	b.clear();
	mut stack := []StackCell{len: 64, init: StackCell { x: (1<<max_level) - 1, k: max_level, w: 0 }}
	mut t := 1;
	for t > 0 {
		t -= 1;
		z := stack[t];
		if z.k <= 3 {
			...
			stack[t] = StackCell { x: z.x + (1<<(z.k-1)), k: z.k - 1, w: 0 };
			t += 1;
		}
	}
}
...
fn main() {
	...
	a1.sort_with_compare(fn (x &Interval, y &Interval) i64 { return x.st - y.st })
